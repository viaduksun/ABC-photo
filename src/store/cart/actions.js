...
// === TEMP ===
export const setCartActionTemp = (singleProduct) => (dispatch) => {
...
export const addToCartMongoDB = (singleProduct) => (dispatch) => {
  ...
  getCartFromDB().then((result) => {
    if (result.data) {
      ...
      const isMatch = arrayFromDB.find(
...
export const deleteFromCartMongoDB = (singleProduct) => (dispatch) => {
  getCartFromDB().then((result) => {
    if (result.data) {
      console.log('CART FROM DB: ', result.data.products);
      const arrayFromDB = result.data.products;
      const newArrayFromDB = arrayFromDB.filter(
        (item) => item.product._id !== singleProduct._id
      );
      console.log(newArrayFromDB);
      const cartCounter = newArrayFromDB.reduce(
...
export const decrementCartMongoDB = (currentID) => (dispatch) => {
  getCartFromDB().then((result) => {
    if (result.data) {
      ...
      console.log(newArrayFromDB);
      const newArrayFromDBFiltered = newArrayFromDB.filter(
        (item) => item.cartQuantity !== 0
      );

      const cartCounter = newArrayFromDBFiltered.reduce(
        (a, b) => a + b.cartQuantity,
        0
      );
...
export const cartDeleteAction = () => (dispatch) => {
  deleteCart().then((result) => {
    console.log('DELETE CART: ', result);
    dispatch({ type: DELETE_CART_DB_REDUX });
  });
};
export const logOutAction = () => localStorage.removeItem('token');
({
  type: LOG_OUT,
});
